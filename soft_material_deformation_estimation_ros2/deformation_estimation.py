...
import model_ViTransformer

input_shape = (1, 1, 224, 224)
model_folder = '/home/kildall/ros2_projects/deformable_ws/src/soft_material_deformation_estimation_ros2/models'
...
class DeformationNode(Node):
    ...
    def __init__(self):
        ...
        segmentation_model_path = os.path.join(model_folder, 'segmentation/best_model.pth')
        # deformation_model_path = os.path.join(model_folder, 'deformation_estimation/epoch52.pth')

        deformation_model_path = os.path.join(model_folder,
                                              'deformation_estimation/ruby-darkness-1_600_1e-05_64_BestEpoch.pth')
        ...
        self.ply_rest_shape_client = self.create_service(srv_type=Trigger, srv_name='get_rest_ply_shape',
                                                         callback=self.get_rest_ply_shape)

        self.ply_rest_shape_reset_client = self.create_service(srv_type=Trigger, srv_name='reset_rest_ply_shape',
                                                               callback=self.reset_rest_ply_shape)
    ...
    def estimate_deformation(self):
        ...
        if self.rest_ply_shape_features is not None:
            ...
            if np.sum(segmented_depth) > 0:
                ...
                prob_deformation: np.array = self.deformation_estimator.fast_predict_classification(
                    self.rest_ply_shape_features,
                    depth).squeeze().cpu().detach().numpy()
                deformation_classes = np.argmax(prob_deformation, axis=-1)
            else:
                prob_deformation = np.zeros((5, 5))
                deformation_classes = np.array([2 for _ in range(self.dofs)])

            if self.publish_probabilities:
                dim1 = MultiArrayDimension()
                dim1.label = "dofs"
                dim1.size = self.dofs  # Number of rows
    ...
    def get_rest_ply_shape(self, request, response: Trigger.Response):
        self.rest_ply_shape[0, 0] = torch.from_numpy(self.get_segmented_depth()).float().to(self.device) / 1000
        self.rest_ply_shape_features = self.deformation_estimator.extract_features(self.rest_ply_shape)
        response.success = True

        self.logger.info('Got new rest ply shape')
        return response

    def reset_rest_ply_shape(self, request, response: Trigger.Response):
        self.rest_ply_shape = torch.zeros(input_shape, dtype=torch.float, device=self.device)
        self.rest_ply_shape_features = None

        response.success = True

        self.logger.info('Reset rest ply shape')
        return response
