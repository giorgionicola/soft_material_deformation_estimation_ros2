...
class MoveToStartNode(Node):
    ...
    def estimate_start_pose(self):
        ...
        j = 0
        # while i < n_frames:
        if not self.tf_buffer.can_transform(self.base_frame, 'tag_10', rclpy.time.Time(), Duration(seconds=0.5)):
            self.logger.warning('Can\'t find tag_10 1')
            return
        if not self.tf_buffer.can_transform(self.base_frame, 'tag_20', rclpy.time.Time(), Duration(seconds=0.5)):
            self.logger.warning('Can\'t find tag_20 1')
            return
        # try:
        t = self.tf_buffer.lookup_transform(self.base_frame, 'tag_10', rclpy.time.Time())
        tag10_pose[:3, 3] = [t.transform.translation.x,
                             t.transform.translation.y,
                             t.transform.translation.z]

        tag10_pose[:3, :3] = R.from_quat([t.transform.rotation.x,
        ...
        # while j < n_frames:
        # self.tf_buffer.wait_for_transform_async(self.base_frame, 'tag_20', self.get_clock().now())
        # try:
        t = self.tf_buffer.lookup_transform(self.base_frame, 'tag_20', rclpy.time.Time())
        tag20_pose[:3, 3] = [t.transform.translation.x,
                             t.transform.translation.y,
                             t.transform.translation.z]

        tag20_pose[:3, :3] = R.from_quat([t.transform.rotation.x,
                                          t.transform.rotation.y,
                                          t.transform.rotation.z,
                                          t.transform.rotation.w], ).as_matrix()
        # except:
        ...
        middle_frame_pos = (tag10_pose[:3, 3] + tag20_pose[:3, 3]) / 2
        ...
        ee_target_pos = [0, self.start_distance + 0.15 + 0.03, 0]
